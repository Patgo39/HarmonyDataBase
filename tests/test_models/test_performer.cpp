...
TEST(TestPerformer, test_default_values) {
  ...
  ASSERT_EQ(performer.getType(), Type::UNKNOWN)
    << "Default value for id_type must be Type::UNKNOWN.";
...
TEST(TestPerformer, test_set_id_type) {
  ...
  try {
    performer.setIntType(-5);
    FAIL() << "An invalid negative id_type was setted and Performer did not throw an invalid_argument.";
  } catch (const std::invalid_argument&) {}

  try {
    performer.setIntType(5);
    FAIL() << "An id_type grater than 3 was setted and Performer did not throw an invalid_argument.";
  } catch (const std::invalid_argument&) {}

  performer.setIntType(2);
  ASSERT_EQ(performer.getType(), Type::GROUP)
