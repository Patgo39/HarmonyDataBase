...
void Person::validateDate(const std::string &date) const {
    ...
    if (!std::regex_match(date, date_pattern)) {
        throw std::invalid_argument("Year has an invalid format. Must be YYYY-MM-DD with valid month/day.");
    }
}
...
std::string Person::getStageName() const { 
  return stage_name; 
}
...
std::string Person::getRealName() const { 
  return real_name; 
}
...
std::string Person::getDeathDate() const { 
  return death_date; 
}
